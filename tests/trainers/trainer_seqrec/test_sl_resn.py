...
from genrec.trainers.trainer_seqrec.sl_resn import SLReSNSeqRecTrainer, SLReSNSeqRecTrainingArguments
...
def test_sl_resn_seqrec_trainer_backward_pass(tmp_path: Path) -> None:
    args = build_training_args(
        tmp_path,
        args_cls=SLReSNSeqRecTrainingArguments,
        sl_temperature=0.3,
        resn_weight=0.4,
    )
...
def test_sl_resn_seqrec_trainer_normalization_branch(tmp_path: Path) -> None:
    args = build_training_args(
        tmp_path,
        args_cls=SLReSNSeqRecTrainingArguments,
        sl_temperature=0.25,
...
def test_sl_resn_seqrec_trainer_sequence_wise_negatives(tmp_path: Path) -> None:
    args = build_training_args(
        tmp_path,
        args_cls=SLReSNSeqRecTrainingArguments,
        sl_temperature=0.35,
